...
爲此，有了一統中文本地化：我自己保留一份主要的翻譯文件 `common.zh_Hant.po`，其他的版本（`zh_CN`, `zh_HK`, `zh_TW`）從 [OpenCC](https://github.com/BYVoid/OpenCC) 優雅地轉換而來。
...
如將來有變動，可爲另外的一位活躍貢獻者「傳授」此倉庫，或乾脆放棄此模式，衆人拾柴。
...
如，「cells」在遊戲中作爲 Electro 等電氣主題武器的彈藥，應當翻譯爲「電池」而非「細胞」。

再如，「capture」在遊戲中的用法分爲「capture flag」和「capture control point」，不能一概而論、混爲一談。需視情況譯爲「奪取」或「佔領」。

- 語法、語序準確，但不失靈活

如，要把原因放在句子主幹前。「之所以……是因爲……」一樣的話出現在激烈的遊戲過程中是要捱打的。

再如，可視語境把被動句變爲主動句。避免晦澀的「翻譯腔」。
